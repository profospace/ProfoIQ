...
const Dashboard = () => {
    ...
    // Filter states for visited, saved, and contacted
    ...
    const filterActivitiesByDate = (propertyId, date) => {
        ...
        let filtered = placeholderActivities.filter(
            activity =>
                activity.propertyId === propertyId &&
                activity.timestamp.startsWith(formattedDate)
        );

        // Apply additional filters for visited, saved, and contacted
        if (visitedFilter !== null) {
            filtered = filtered.filter(activity => activity.activity === 'Visited' === visitedFilter);
        }
        if (savedFilter !== null) {
            filtered = filtered.filter(activity => activity.saved === savedFilter);
        }
        if (contactedFilter !== null) {
            filtered = filtered.filter(activity => activity.contacted === contactedFilter);
        }
            ...
            {selectedPropertyId && (
                <div className="mt-4">
                    ...
                    <div className="mt-3">
                        ...
                        {/* Filtering UI for visited, saved, and contacted */}
                        <div>
                            <button onClick={() => setVisitedFilter(visitedFilter === null ? true : visitedFilter === true ? false : null)} className="btn btn-outline-primary">
                                {visitedFilter === null ? 'All Visits' : visitedFilter ? 'Visited: Yes' : 'Visited: No'}
                            </button>
                            <button onClick={() => setSavedFilter(savedFilter === null ? true : savedFilter === true ? false : null)} className="btn btn-outline-secondary ml-2">
                                {savedFilter === null ? 'All Saved' : savedFilter ? 'Saved: Yes' : 'Saved: No'}
                            </button>
                            <button onClick={() => setContactedFilter(contactedFilter === null ? true : contactedFilter === true ? false : null)} className="btn btn-outline-success ml-2">
                                {contactedFilter === null ? 'All Contacted' : contactedFilter ? 'Contacted: Yes' : 'Contacted: No'}
                            </button>
                        </div>
